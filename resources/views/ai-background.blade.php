...
<body class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 min-h-screen flex flex-col items-center py-12 px-4">
    ...
    <h1 class="text-3xl font-bold mb-8 text-center text-indigo-700">AI Background Generator</h1>
    ...
    <form action="{{ route('ai.background.process') }}" method="POST" enctype="multipart/form-data" class="w-full max-w-lg bg-white p-6 rounded shadow">
        @csrf
        <div class="mb-4">
            <label for="image" class="block text-sm font-semibold mb-2">Upload Transparent Image</label>
            <input type="file" name="image" id="image" required class="w-full border border-gray-300 rounded px-4 py-2">
        </div>

        <div class="mb-4">
            <label for="prompt" class="block text-sm font-semibold mb-2">Prompt</label>
            <input type="text" name="prompt" id="prompt" required class="w-full border border-gray-300 rounded px-4 py-2" placeholder="e.g., A lush green forest with sunlight filtering through the trees">
        </div>

        <div class="mb-4">
            <label for="style" class="block text-sm font-semibold mb-2">Style (optional)</label>
            <select name="style" id="style" class="w-full border border-gray-300 rounded px-4 py-2">
        ...
        <div class="mb-4">
            <label for="seed" class="block text-sm font-semibold mb-2">Seed (optional)</label>
            <input type="number" name="seed" id="seed" class="w-full border border-gray-300 rounded px-4 py-2" placeholder="e.g., 12">
        </div>
    ...
    @isset($image)
        <div class="mt-10 text-center">
            <h2 class="text-xl font-semibold text-green-600 mb-4">Result:</h2>
            <img id="resultImage" src="data:image/png;base64,{{ $image }}" 
                 alt="AI Background Added" 
                 class="rounded-lg shadow-lg max-w-xl mx-auto cursor-pointer hover:scale-105 transition" 
                 title="Click to view fullscreen" />

            <a href="data:image/png;base64,{{ $image }}" download="ai-background-{{ time() }}.png"
